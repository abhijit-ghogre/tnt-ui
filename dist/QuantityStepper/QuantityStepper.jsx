...
function QuantityStepper(props) {
    ...
    return (<div className={`flex border w-fit overflow-hidden ${className}`}>
      <button className={`btn btn-ghost btn-xs no-animation rounded-r-none
          ${getButtonSizeClass(size)}
        `} onClick={() => handleChange(`${value - 1}`)} disabled={value <= min}>
        <md_1.MdRemove />
      </button>
      <input onChange={(e) => handleChange(e.target.value)} type="text" placeholder="10" className={`input focus:bg-transparent text-center px-1 rounded-none input-xs 
      ...
      <button className={`btn btn-ghost btn-xs no-animation rounded-l-none
          ${getButtonSizeClass(size)}
